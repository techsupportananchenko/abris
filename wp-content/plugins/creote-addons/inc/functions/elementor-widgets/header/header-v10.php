    ...
    $this->add_control(
       ...
       $this->add_control(
         ...
         $this->add_control(
           'search_color',
            [
               'label' => __('Search Placeholder / Input Color', 'creote-addons'),
               'type' => Controls_Manager::COLOR,
                'selectors' => [
                   '{{WRAPPER}} .header_style_ten .serch_simple input::placeholder , {{WRAPPER}} .header_style_ten .serch_simple input ' => 'color: {{VALUE}}!important;',
                 ],
            ]
         );  
         $this->add_control(
           'si_border_color',
            [
               'label' => __('Search Border Color', 'creote-addons'),
               'type' => Controls_Manager::COLOR,
                'selectors' => [
                   '{{WRAPPER}} .header_style_ten .serch_simple input ' => 'border-color: {{VALUE}}!important;',
                 ],
            ]
         );
         $this->add_control(
           'sb__color',
            [
               ...
               'type' => Controls_Manager::COLOR,
               'selectors' => [
                   '{{WRAPPER}} .header_style_ten .serch_simple button ' => 'color: {{VALUE}}!important;',
               ],
            ]
         );
         $this->add_control(
       ...
       $this->add_control(
           'btncolor',
            [
               ...
               'type' => Controls_Manager::COLOR,
               'selectors' => [
                   '{{WRAPPER}} .header_style_ten .con_button a ' => 'color: {{VALUE}};',
               ],
            ]
       );
       $this->add_control(
        'btnbgcolors',
         [
            ...
            'type' => Controls_Manager::COLOR,
            'selectors' => [
                '{{WRAPPER}} .header_style_ten .con_button a ' => 'background: {{VALUE}}; border-color: {{VALUE}}; ',
            ],
         ]
    );
    $this->add_control(
        'btncholor',
         [
            ...
            'type' => Controls_Manager::COLOR,
            'selectors' => [
                '{{WRAPPER}} .header_style_ten .con_button a:hover ' => 'color: {{VALUE}};',
            ],
         ]
    );
    $this->add_control(
     'btnbgcohlor',
      [
         ...
         'type' => Controls_Manager::COLOR,
         'selectors' => [
             '{{WRAPPER}} .header_style_ten .con_button a:hover ' => 'background: {{VALUE}};  border-color: {{VALUE}}; ',
         ],
      ]
 );
       $this->end_controls_section();
       ...
       protected function render()
       {
           ...
           ?>
<div class="header_style_six_nw header_style_ten"> <?php if($settings['top_bar_enable'] == 'yes'): ?> <div class="top_bar style_six"> <div class="auto-container"> <div class="row"> <div class="col-lg-12"> <div class="top_inner"> <div class="left_side common_css"> <?php if(!empty($settings['notices'])): ?> <p class="notice"><?php echo wp_kses($settings['notices'] , $allowed_tags); ?></p> <?php endif; ?> <?php if($settings['list_content_enable'] == 'yes'): ?> <ul> <?php foreach($settings['list_content_repeater'] as $list_content_repeater): $target = $list_content_repeater['link']['is_external'] ? ' target="_blank"' : ''; $nofollow = $list_content_repeater['link']['nofollow'] ? ' rel="nofollow"' : ''; ?> <li><a href="<?php echo esc_url($list_content_repeater['link']['url']) ?>" <?php echo esc_attr($target); ?> <?php echo esc_attr($nofollow); ?>> <?php echo wp_kses($list_content_repeater['list_itme_text'] , $allowed_tags); ?></a> </li> <?php endforeach; ?> <ul> <?php endif; ?> </div> <div class="right_side common_css"> <div class="rg_content_box"> <?php if($settings['search_enable'] == 'yes'): ?> <div class="serch_simple"> <?php creote_simple_search(); ?> </div> <?php endif; ?> <?php if($settings['scoial_media_enable'] == 'yes'): ?> <div class="med_rg_side"> <ul> <?php foreach($settings['social_media_repeater'] as $media_repearter):?> <?php $target_three = $media_repearter['socail_media_link']['is_external'] ? ' target="_blank"' : ''; $nofollow_three = $media_repearter['socail_media_link']['nofollow'] ? ' rel="nofollow"' : ''; ?> <li> <a href="<?php echo esc_url($media_repearter['socail_media_link']['url']); ?>" <?php echo esc_attr($target_three); ?> <?php echo esc_attr($nofollow_three); ?>> <i class="<?php echo esc_attr($media_repearter['social_media_icon']); ?>"></i> </a> </li> <?php endforeach;?> </ul> </div> <?php endif; ?> </div> </div> </div> </div> </div> </div> </div> <?php endif;?> <header class="header header_default"> <div class="auto-container"> <div class="row align-items-center"> <div class="col-lg-2 col-md-9 col-sm-9 col-xs-9 logo_column"> <div class="header_logo_box"> <a href="<?php echo esc_url($url); ?>" class="logo navbar-brand" <?php echo esc_attr($logo_target); ?> <?php echo esc_attr($logo_nofollow); ?>> <img src="<?php echo esc_url($settings['logo_default']['url']); ?>" alt="<?php echo esc_html(get_bloginfo( 'name' )); ?>" class="logo_default"> </a> </div> </div> <div class="col-lg-10 col-md-3 col-sm-3 col-xs-3 menu_column"> <div class="navbar_togglers hamburger_menu"> <span class="line"></span> <span class="line"></span> <span class="line"></span> </div> <div class="header_content_collapse"> <div class="header_menu_box"> <div class="navigation_menu"> <?php if(!empty($settings['navigations'])): wp_nav_menu(array( 'menu' => $settings['navigations'], 'container' => false, 'menu_class' => 'navbar_nav', 'menu_id' => 'myNavbar', 'fallback_cb' => 'WP_Bootstrap_Navwalker::fallback', 'walker' => new \WP_Bootstrap_Navwalker() ) ); endif; ?> </div> </div> <div class="header_right_content"> <ul> <?php if(class_exists('woocommerce')): if($settings['cart_enable'] == 'yes'): ?> <li> <?php $items_counts = is_object( WC()->cart ) ? WC()->cart->get_cart_contents_count() : ''; ?> <div class="mini_cart_togglers header_side_cart"> <div class="mini-cart-count"> <?php if(!empty($items_counts)): echo $items_counts ? $items_counts : '&nbsp;'; else: echo esc_html('0'); endif; ?> </div> <i class="icon-shopping-bag1"></i> </div> </li> <?php endif; endif;?> <?php if($settings['modal_enable'] == 'yes'): ?> <li> <button type="button" class="contact-toggler"><i class="icon-menu1"></i></button> </li> <?php endif;?> <?php if($settings['button_enable'] == 'yes'): ?> <li class="con_button"> <?php $target_two = $settings['button_link']['is_external'] ? ' target="_blank"' : ''; $nofollow_two = $settings['button_link']['nofollow'] ? ' rel="nofollow"' : ''; ?> <a href="<?php echo esc_url($settings['button_link']['url']) ?>" class="theme-btn one" <?php echo esc_attr($target_two); ?> <?php echo esc_attr($nofollow_two); ?>> <?php echo wp_kses($settings['button_text'] , $allowed_tags); ?></a> </li> </li> <?php endif; ?> </ul> </div> </div> </div> </div> </div> </header></div>
<?php
}
}
