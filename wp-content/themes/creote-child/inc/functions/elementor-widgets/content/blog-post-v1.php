...
class Widget_creote_blog_post_grid_v1 extends Widget_Base {
	...
	protected function render() {
		...
		?>
		<section
			class="blog_post_section elemen_wp <?php echo esc_attr( $css_classes ); ?> <?php echo esc_attr( $settings['news_styles'] ); ?> <?php if ( $settings['masonory_enable'] == 'yes' ) : ?> masonary_enable <?php endif; ?>">
			<div class="grid_show_case grid_layout clearfix"> <!-- swiper-slide -->
				...
				$args = array( 'post_type' => 'post', 'ignore_sticky_posts' => true, 'orderby' => 'date', 'paged' => $paged, 'posts_per_page' => $settings['post_count'], 'orderby' => $settings['query_orderby'], 'order' => $settings['query_order'], );
				if ( $settings['query_category'] )
					$args['category_name'] = $settings['query_category'];
				...
				<?php while ( $blog_grid_query->have_posts() ) : ?>
					...
					<?php if ( $settings['news_styles'] == 'style_two' ) : ?>
						<div class="grid_box _card">
							<div class="news_box default_style list_view <?php if ( $settings['normal_view'] == 'yes' ) : ?> normal_view <?php endif; ?> <?php if ( has_post_thumbnail() ) : ?>has_images<?php else : ?>no_images<?php endif; ?>"
								id="post-<?php esc_attr( the_ID() ); ?>"> <?php if ( has_post_thumbnail() ) : ?>
									<div class="image img_hover-1"> <?php the_post_thumbnail(); ?> 					<?php creote_category(); ?> </div>
								<?php endif; ?>
								<div class="content_box">
									<div class="date"> <span
											class="date_in_number"><?php echo esc_attr( get_the_date( get_option( 'date_format' ) ) ); ?></span>
									</div>
									<div class="source">
										...
										<?php if ( ! empty( $myexcerpt ) ) : ?>
											...
											class="link__go"><?php echo esc_attr( $settings['button_text'] ); ?></a>
									</div>
									<div class="auhtour_box"> <?php echo get_avatar( get_the_author_meta( 'ID' ), 60 ); ?>
										<div class="contnet_a">
						...
						</div> <?php elseif ( $settings['news_styles'] == 'style_three' ) : ?>
						...
						</div> <?php elseif ( $settings['news_styles'] == 'style_four' ) : ?>
						<div class="grid_box _card style_man">
							<div class="news_box style_four <?php if ( $settings['border_radius_enable'] == 'yes' ) : ?>no_radius<?php endif; ?> <?php if ( has_post_thumbnail() ) : ?>has_images<?php else : ?>no_images<?php endif; ?>"
								id="post-<?php esc_attr( the_ID() ); ?>" <?php if ( has_post_thumbnail() ) : ?>
									style="background-image: url('<?php the_post_thumbnail_url(); ?>');" <?php endif; ?>>
								<div class="overlay"> </div>
								<div class="date"> <span class="date_in_month"><?php echo esc_attr( get_the_date( 'M' ) ); ?></span> <span
										class="date_in_number"><?php echo esc_attr( get_the_date( 'd' ) ); ?></span> </div>
								<div class="content_box">
									<div class="category"> <?php creote_category(); ?> </div>
									<?php the_title( '<h2 class="title"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h2>' ); ?>
								</div>
						...
						</div> <?php elseif ( $settings['news_styles'] == 'style_five' ) : ?>
						<div class="grid_box _card">
							<div class="news_box style_five">
								<div class="content_box">
									<ul>
										...
										<li> <a href="#"><span
													class="fa fa-clock-o"></span><?php echo esc_attr( get_the_date( get_option( 'date_format' ) ) ); ?></a>
										</li>
									</ul>
									<?php the_title( '<h2 class="title"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h2>' ); ?>
									<?php if ( ! empty( $myexcerpt ) ) : ?>
						...
						</div> <?php elseif ( $settings['news_styles'] == 'style_six' ) : ?>
						...
						<div class="grid_box _card">
							<div class="news_box style_seven"> <?php if ( has_post_thumbnail() ) : ?>
						...
						<div class="grid_box _card">
							<div class="news_box style_eight"> <?php if ( has_post_thumbnail() ) : ?>
						...
						</div> <?php elseif ( $settings['news_styles'] == 'style_nine' ) : ?>
						<div class="grid_box _card">
							<div class="news_box style_nine">
								<div class="content_box">
									<div class="date"> <span class="date_in_number"><?php echo esc_attr( get_the_date( 'd' ) ); ?></span>
										<span class="date_in_month"><?php echo esc_attr( get_the_date( 'M' ) ); ?></span> </div>
						...
						</div> <?php elseif ( $settings['news_styles'] == 'style_ten' ) : ?>
						<div class="grid_box _card">
							<div class="news_box style_ten"> <?php if ( has_post_thumbnail() ) : ?>
									<div class="image_box"> <?php the_post_thumbnail(); ?> </div> <?php endif; ?>
								<?php the_title( '<h2 class="title"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h2>' ); ?>
								<ul>
									<li class="cag"> <?php creote_category(); ?> </li>
									<li class="dte"><i
											class="icon-clock"></i><?php echo esc_attr( get_the_date( get_option( 'date_format' ) ) ); ?></li>
								</ul>
							</div>
						</div> <?php else : ?>
						<div class="grid_box _card">
							<div class="news_box style_one blog_classic <?php if ( has_post_thumbnail() ) : ?>has_images<?php else : ?>no_images<?php endif; ?>"
								id="post-<?php esc_attr( the_ID() ); ?>"> <?php if ( has_post_thumbnail() ) : ?>
									<div class="image img_hover-1"> <?php the_post_thumbnail( $image_size ); ?> <a class="arrow"
											href="<?php echo esc_url( get_permalink() ); ?>"><i class="fa fa-angle-right"></i></a> </div>
								<?php endif; ?>
								<div class="content_box">
									<div class="date"> <span class="date_in_number"><?php echo esc_attr( get_the_date( 'd' ) ); ?></span>
										<span class="date_in_month"><?php echo esc_attr( get_the_date( 'M' ) ); ?></span> </div>
						...
						</div> <?php endif; ?> 		<?php endwhile; ?> 		<?php wp_reset_postdata(); ?>
			</div> <?php if ( $settings['pagination_enable'] == true ) : ?>
				<div class="row">
					<div class="col-lg-12">
						<div class="pagination">
							<?php $pagination = 999999999;
							echo paginate_links( array( 'base' => str_replace( $pagination, '%#%', get_pagenum_link( $pagination ) ), 'format' => '?paged=%#%', 'current' => max( 0, $paged ), 'total' => $blog_grid_query->max_num_pages, 'prev_text' => '<i class="fa fa-angle-left"></i>', 'next_text' => '<i class="fa fa-angle-right"></i>', 'type' => 'list', 'add_args' => false ) ); ?>
						</div>
					</div>
				</div> <?php endif; ?>
		</section>
